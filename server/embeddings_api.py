...
def embed_texts(
    ...
    """Embed text with a local embedding model or a configured online provider."""
    try:
        ...
        if embed_model in list_online_embed_models():  # 使用在线API
            ...
            configured_embed_model = config.get("embed_model")
            if worker_class and worker_class.can_embedding():
                # ApiModelWorker requires model_names even when it is used directly,
                # rather than through a FastChat model-worker process.
                worker = worker_class(model_names=[embed_model], no_register=True)
                params = ApiEmbeddingsParams(
                    texts=texts,
                    to_query=to_query,
                    embed_model=configured_embed_model,
...
async def aembed_texts(
    ...
    """Asynchronous wrapper for embedding requests.

    Online workers are synchronous by design.  Run them in FastAPI's thread pool
    instead of calling a non-existent ``ado_embeddings`` method.
    """
    try:
