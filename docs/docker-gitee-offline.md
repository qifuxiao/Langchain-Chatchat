...
首次启动会从模板生成 `configs/*.py`，并读取 `GITEE_AI_*` 环境变量。默认文档切分器使用本地 `tiktoken`，镜像同时内置 Word 文档解析所需的 NLTK `punkt`、`punkt_tab` 和 `averaged_perceptron_tagger_eng` 数据；不会请求 Hugging Face，离线运行时不需要再下载。知识库、FAISS 索引和 SQLite 元数据保存到 `./data/knowledge_base`，日志保存到 `./data/logs`；升级镜像不会覆盖这些数据。
